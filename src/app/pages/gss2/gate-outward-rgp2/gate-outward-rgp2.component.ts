...
@Component({
  selector: "app-gate-outward-rgp2",
  templateUrl: "./gate-outward-rgp2.component.html",
  styleUrls: ["./gate-outward-rgp2.component.scss"],
})
export class GateOutwardRGP2Component implements OnInit {
  ...
  fileUploadUrls: any[] = [];
  createNewFile: any = {};
  fileUploadUrlsRGP2: any[] = [];
  selectedfiles: any[] = [];
  filenamearray: any[] = [];
  editdataa: any;
  deleteNumber: any;
  resultRGP2number: any;
  RGP2number: any;
  filedata: any;
  deleteid: any;
  dialogRef: any = null;
...
this.custservice.getGateOutwardRGP2dropdowndata(obj).subscribe((res:any)=>{
...
this.model1.documentnumber=""
  ...
  getgoodsissuedata() {
    ...
    this.custservice.printoutgatepassRgpData2(obj).subscribe((res: any) => {
      // this.userData = res.data;
      this.RGP2data = res.data;
...
this.model1.expectedredate = moment(res.data[0].expected_return_date).format("YYYY-MM-DD")
  ...
  savefinaldata(fr: any) {
    if (this.btn === "Save") {
      ...
      this.custservice.addGateOutwordRGP2(obj).subscribe((res: any) => {
        if (res && res["status_code"] == "200") {
          ...
          this.smsdata = "";
          this.getRGP2ListData();
          this.resultRGP2number = res["reference"];
          if (this.fileUploadUrlsRGP2.length > 0) {
  ...
  editRGP2data(row1: any, index: any, data: any) {
    console.log(row1);
  ...
  getRGP2ListData() {
    ...
    this.custservice.getGateOutwardRGP2List(obj).subscribe((res: any) => {
      console.log(res);
      this.reaData = false;
  ...
  deleteFile() {
    ...
    this.custservice.deleteGateOutwardRGP2Data(obj).subscribe((res: any) => {
      if (res && res["status_code"] == "200") {
        this.alertcall.showSuccess("Accepted", res["message"]);
  ...
  openfileuploadmodel(data: any, row1: any) {
    ...
    this.RGP2number = row1.number;
    this.getexistingfiles();
  }
  getexistingfiles() {
    let params = new HttpParams();
    params = new HttpParams()
      .set("document_number", this.RGP2number)
      .set("document_type", "Gate Outward RGP2");
  ...
  uploadWbsFile(fileInput: any) {
    ...
    postData.append("document_type", "Gate Outward RGP2");
    postData.append("document_number", this.RGP2number);
    for (const file of this.fileUploadUrls) {
  ...
  uploadRGPfiles(fileInput: any) {
    if (
      ...
      this.fileUploadUrlsRGP2 = fileInput.target.files;
      this.createNewFile.fileName = fileInput.target.files[0].name;
      for (const file of this.fileUploadUrlsRGP2) {
  ...
  uploadselectedfiles() {
    const postData = new FormData();
    postData.append("document_type", "Gate OutwardRGP2");
    postData.append("document_number", this.resultRGP2number);
    for (const file of this.selectedfiles) {
      postData.append("doc", file);
    }
    this.custservice.addfileupload(postData).subscribe((res: any) => {
      if (res && res["status_code"] == "200") {
        this.fileUploadUrlsRGP2 = [];
        this.filenamearray = [];
        this.selectedfiles = [];
      } else {
      }
  ...
  search() {
    ...
    if (this.searchData.length > 2) {
      this.custservice
        .getGateOutwardRGP2searchData(obj)
        .subscribe((res: any) => {
          this.reaData = false;
          this.totalRecords = res?.count;
          this.dataSourcemain.data = res.data;
