...
export class ProjectFoldersComponent implements OnInit {
  ...
  getsearchdata() {
    ...
    this.customerService.searchProjectPdfs(params).subscribe((res: any) => {
  ...
  getPdfFileVersions() {
    this.customerService.getPdfFileVersions().subscribe((resp: any) => {
      if (resp && resp.status_code === 200) {
        ...
        this.versions$1 = this.versions$.slice(0, 22);
      }
    });
  }
  ...
  getFolders(parent: any) {
    ...
    this.customerService.getFolders(params).subscribe(
      (resp: any) => {
        if (resp && resp.status_code === 200) {
  ...
  saveFolder() {
    ...
    this.customerService.createFolder(params).subscribe((resp: any) => {
      if (resp && resp.status_code === 200) {
  ...
  backToFolder(folder: any, folderIndex: number) {
    ...
    this.folderInnerDetails.forEach((element: any, index: number) => {
      if (index >= folderIndex + 1) {
        // this.folderInnerDetails.splice(index, 1);
    ...
    this.customerService.getFolders(params).subscribe((resp: any) => {
      if (resp && resp.status_code === 200) {
        this.folders$ = resp.data;
        this.selectedArray=[]
      } else if (resp && resp.status_code === 304) {
  ...
  deleteFolder() {
    ...
    this.customerService.deleteFolder(params).subscribe((resp: any) => {
      if (resp && resp.status_code === 200) {
        this.dialog.closeAll();
  ...
  public downloadpdf(): void {
    ...
    this.pdfFiles$.forEach((element: any, index: any) => {
      array.push([
        ...
        element.name,
        element.drawing_no,
        element.drawing_type,
        element.version,
        element.created_by,
        moment(element.created_date).format("MMM DD,YYYY"),
        element.comments,
      ]);
    });
  ...
  // givenname:any
  // allUsers$:any[]=[]
  // filterUserName(ev:any, val: string){
  //   if (!val) {
  //     return [];
  //   }
  ...
  //     this.selectFormControl.value.forEach((ele:any) => {
  //       if (this.selectedValues.indexOf(ele) == -1) {
