...
@Component({
  selector: 'app-external-drawings',
  templateUrl: './external-drawings.component.html',
  styleUrls: ['./external-drawings.component.scss']
})
export class ExternalDrawingsComponent implements OnInit {
  projectId: any;
  projectObj: any = {};
  startDate: any;
  endDate: any;
  ...
  searchfolders(){
    ...
    .set('version',this.searchversion)
      ...
      this.customerService.searchExternalFolders(params).subscribe((res:any)=>{
  ...
  getFolders(parent: any) {
    ...
    this.customerService.getExtraFolders(params).subscribe(
      (resp: any) => {
        if (resp && resp.status_code === 200) {
  ...
  saveFolder() {
    ...
    this.customerService.createExtraFolder(params).subscribe((resp: any) => {
      if (resp && resp.status_code === 200) {
  ...
  backToFolder(folder: any, folderIndex: number) {
    ...
    this.customerService.getExtraFolders(params).subscribe((resp: any) => {
      if (resp && resp.status_code === 200) {
        this.folders$ = resp.data;
        this.selectedArray=[]
      } else if (resp && resp.status_code === 304) {
  ...
  getFolderDocs() {
    ...
    this.customerService.getExtraPdfFiles(params).subscribe((resp: any) => {
      if (resp && resp.status_code === 200) {
  ...
  deleteFolder() {
    ...
    this.customerService.deleteExtraFolder(params).subscribe((resp: any) => {
      if (resp && resp.status_code === 200) {
        this.dialog.closeAll();
  ...
  saveFolderFile() {
    ...
    this.customerService.uploadExtraPdfFiles(postData).subscribe((resp: any) => {
      if (resp && resp.status_code === 200) {
        this.fileUploadUrls = [];
        this.createNewFile = {};
        this.selectedUsers = [];
