...
@Component({
  selector: 'app-fuereceiptnote',
  templateUrl: './fuereceiptnote.component.html',
  styleUrls: ['./fuereceiptnote.component.scss']
})
export class FuereceiptnoteComponent implements OnInit {
  ...
  getcfrmData() {
    ...
    this.custService.getconfrimationnodata(obj).subscribe((res: any) => {
  ...
  savefinaldata(fr: any) {
    if (this.btn === "Save") {
      ...
      this.custService.addfuelReceiptData(obj).subscribe((res: any) => {
        console.log(res);
    ...
    if (this.btn === "Update") {
      ...
      this.custService.updateFuelReceipt(obj).subscribe((res: any) => {
        if (res && res["status_code"] == "200") {
          Swal.fire({
            text: res["message"],
            title: res["reference"],
  ...
  getFuelReceiptList() {
    ...
    this.custService.getFuelReceiptData(obj).subscribe((res: any) => {
      console.log(res);
      this.reaData = false;
  ...
  deleteFile() {
    ...
    this.custService.deleteFuelReceiptFile(obj).subscribe((res: any) => {
      if (res && res["status_code"] == "200") {
        this.alertcall.showSuccess("Accepted", res["message"]);
  ...
  uploadedselctedfiles() {
    ...
    postData.append("document_type", "Fuel Receipt Note");
  ...
  getexistingfiles() {
    ...
    params = new HttpParams()
      ...
      .set("document_type", "Fuel Receipt Note");
