...
export class PurchaserequestComponent implements OnInit {
  ...
  displayedColumnsList: any[] = [
    "sno", "MSRNnum", "date", "cmpnyName", "action"
  ]
  demo1TabIndex: any = 0;
  dialogdata: any = {};
  ...
  deleteItem() {
    ...
    this.custservice.deletepurchaserequestdata(obj).subscribe((res: any) => {
      console.log(obj);
      if (res && res['status_code'] == "200") {
        this.alertcall.showSuccess('Accepted', res['message']);
        this.dialog.closeAll()
  ...
  savefinaldata(fr: any) {
    if (this.btn === 'Save') {
      ...
      this.custservice.addpurchaserequestdata(obj).subscribe((res: any) => {
        if (res && res['status_code'] == "200") {
          Swal.fire({
            text: res['message'],
            title: res['reference'],
  ...
  printpurchasereq(data: any) {
    ...
    this.router.navigate(['/planning/printpurchasereq'], { queryParams: { 'dmrumber': data.number } })
  }
  ...
  search(){
   ...
   let obj= {"command":"lst","field":this.columnname ,"key":this.searchData,"lmt":this.pageSize,"pid":this.pageIndex}
    if(this.searchData.length>2){
      this.custservice. getSearchPurchaseRequest(obj).subscribe((res: any) => {
        this.reaData = false;
        this.totalRecords = res?.count;
        this.dataSourceList.data = res.data
        if (res.data.length == 0) {
